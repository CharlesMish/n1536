...
pass('Eighteen numbered studies have matching book entries and valid specimens',()=>{
 ...
 assert(a.length===18&&b.length===18);for(let i=0;i<18;i++){const n=String(i+1).padStart(2,'0');assert(a[i].study===n&&b[i].n===n&&a[i].file===b[i].file);assert(fs.readFileSync(root+a[i].file,'utf8').toUpperCase().includes('FIELD STUDY '+n));assert(figs['p'+n]===specs[n]&&!specs[n].includes('undefined'));}
});
