...
function plotSVG(index) {
  const dataset = DATASETS[index], stats = STATS[index];
  const width = Math.max(260, $('plot' + index).clientWidth);
  const height = Math.max(245, Math.min(300, width * 0.54 + 36));
  const left = 43, right = width - 19, top = 30, bottom = height - 36;
  const x = value => left + value / 20 * (right - left);
  const y = view === 'scatter'
  ...
  geometry[index] = dataset.points.map((point, i) => ({ x: x(point.x), y: y(view === 'scatter' ? point.y : stats.residuals[i]), row: point.row }));
  let svg = `<svg viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="fitTitle${index} fitDesc${index}" data-view="${view}"><title id="fitTitle${index}">Dataset ${dataset.id}: ${view === 'scatter' ? 'points and fitted line' : 'residuals versus x'}</title><desc id="fitDesc${index}">Eleven observations on fixed axes shared with the other datasets. Selected row ${row}: x ${selected.x}, y ${selected.y.toFixed(2)}, fitted y ${fixed(stats.fitted[row - 1], 3)}, residual ${signed(residual)}. Use the Inspect row selector for keyboard inspection.</desc>`;
  ...
  for (const value of view === 'scatter' ? [0, 5, 10, 15] : [-4, -2, 0, 2, 4]) svg += `<path class="grid-line" d="M${left} ${y(value)}H${right}"/><text x="${left - 10}" y="${y(value) + 4}" text-anchor="end">${String(value).replace('-', '−')}</text>`;
  svg += `<path class="axis-line" d="M${left} ${top}V${bottom}H${right}"/><text class="axis-title" x="${left}" y="17">${view === 'scatter' ? 'y' : 'y − ŷ'}</text><text class="axis-title" x="${right}" y="${height - 4}" text-anchor="end">x →</text>`;
  if (view === 'scatter') {
  ...
  geometry[index].forEach(point => { svg += marker(index, point.x, point.y, 4.5, 'observation'); });
  const point = geometry[index][row - 1];
  ...
  geometry[index].forEach(point => { svg += `<circle class="point-target" cx="${point.x}" cy="${point.y}" r="15"/>`; });
  return svg + '</svg>';
}
...
$('openAudit').addEventListener('click', () => { $('audit').open = true; });
if (location.hash === '#audit') $('audit').open = true;
...
$('inspectIII').addEventListener('click', () => inspectRow(3, 'Row 3 selected. Dataset III’s point at (13, 12.74) has residual ' + signed(STATS[2].residuals[2]) + '.'));
$('inspectIV').addEventListener('click', () => inspectRow(8, 'Row 8 selected. Dataset IV’s point at (19, 12.50) has zero residual to displayed precision, yet is the only point away from x = 8.'));
...
DATASETS.forEach((dataset, index) => $('plot' + index).addEventListener('click', event => {
  ...
  const nearest = geometry[index].reduce((best, point) => {
